...
if [[ $EUID -ne 0 ]]; then
   echo "You need root privileges to run this script." 1>&2
    ...
    BASE_DIR=/webapps/junction
    ENV_DIR=$BASE_DIR/env
    PROJECT_DIR=$BASE_DIR/junction
    SETTINGS_DIR=$PROJECT_DIR/junction
    ...
    pkill gunicorn
    $ENV_DIR/bin/gunicorn -c $SETTINGS_DIR/gunicorn.conf.py junction.wsgi_production &
